...
import {MCTSWorkerPoolScoring} from './mcts-worker-pool-scoring';

export class MCTSScoring {

  rootNode: Node;
  currentNode: Node;
  canExecute: Boolean = false;
  workerPool: MCTSWorkerPoolScoring = new MCTSWorkerPoolScoring();
  ...
  static chooseRandomMove(board: Board): Move {
    ...
    if (possibleMoves.length > 0) {
      return MCTSScoring.chooseRandom<Move>(possibleMoves);
    } else {
      return null;
    }
  }
...
console.log(this.currentNode.getAllChildren());
  ...
  private playRandomGame(node: Node): number {
    ...
    while (winner === 0) {
      const move: Move = MCTSScoring.chooseRandomMove(board);
      board.makeMove(move);
